...
use cosmwasm_std::{
    attr, ensure, Addr, Attribute, Decimal, DepsMut, Event, StdError, Storage, Uint128,
};

/// 100% represented as basis points
const MAX_BASIS_POINTS: u128 = 10000u128;

pub struct Pool(PoolConfig);
    ...
    pub fn create_event(
        ...
        for attr_keys in attr_keys {
            let attribute = match attr_keys {
                "collection" => attr("collection", self.0.collection.to_string()),
                "owner" => attr("owner", self.0.owner.to_string()),
                ...
                "pool_type" => attr("pool_type", self.0.pool_type.to_string()),
                "bonding_curve" => attr("bonding_curve", self.0.bonding_curve.to_string()),
                "spot_price" => attr("spot_price", self.0.spot_price.to_string()),
                "delta" => attr("delta", self.0.delta.to_string()),
                "total_nfts" => attr("total_nfts", self.0.total_nfts.to_string()),
                "is_active" => attr("is_active", self.0.is_active.to_string()),
                "swap_fee_percent" => attr("swap_fee_percent", self.0.swap_fee_percent.to_string()),
                "finders_fee_percent" => {
                    attr("finders_fee_percent", self.0.finders_fee_percent.to_string())
                },
                "reinvest_tokens" => attr("reinvest_tokens", self.0.reinvest_tokens.to_string()),
                "reinvest_nfts" => attr("reinvest_nfts", self.0.reinvest_nfts.to_string()),
                _key => {
