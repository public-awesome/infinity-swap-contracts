...
// use crate::state::{
//     buy_pool_quotes, pools, sell_pool_quotes, BondingCurve, Pool, PoolType, POOL_COUNTER,
// };
...
// use crate::testing::tests::helpers::assert_error;
...
// #[test]
// fn validate_pool_quote_index() {
//     let mut deps = mock_dependencies();
//     POOL_COUNTER.save(&mut deps.storage, &1).unwrap();

//     // Token Linear pools get indexed correctly
//     let pool_counter = get_next_pool_counter(&mut deps.storage).unwrap();
//     let test_spot_price = Uint128::from(101u128);
//     let pool = Pool::new(
//         pool_counter,
...
//     );
//     assert!(save_pool(&mut deps.storage, &pool).is_ok());
//     let pool_quote = buy_pool_quotes().load(&deps.storage, pool_counter).unwrap();
//     assert_eq!(test_spot_price, pool_quote.quote_price);
// }
