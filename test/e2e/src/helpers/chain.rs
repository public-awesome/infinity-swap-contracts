...
use super::instantiate::{instantiate_base_factory, instantiate_marketplace};
...
fn global_setup() -> Cfg {
    ...
    orc.poll_for_n_blocks(1, Duration::from_millis(20_000), true).unwrap();

    let skip_storage = env::var("SKIP_CONTRACT_STORE").unwrap_or_else(|_| "false".to_string());
    if !skip_storage.parse::<bool>().unwrap() {
        orc.store_contracts("../../artifacts", &accounts[0].key, None).unwrap();
...
fn save_gas_report(orc: &CosmOrc<CosmosgRPC>, gas_report_dir: &str) {
    let report = orc.gas_profiler_report().expect("error fetching profile reports");
