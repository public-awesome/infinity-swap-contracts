...
use super::constants::{
    BASE_FACTORY_NAME, BASE_MINTER_NAME, CREATION_FEE, INFINITY_INDEX_NAME, INFINITY_ROUTER_NAME,
    LISTING_FEE, MARKETPLACE_NAME, MINT_PRICE, SG721_NAME,
};
...
pub fn instantiate_minter(
    ...
    let tags = res.res.find_event_tags("instantiate".to_string(), "_contract_address".to_string());

    let (minter_addr, sg721_addr) = (tags[0].value.to_string(), tags[1].value.to_string());
    orc.contract_map.add_address(BASE_MINTER_NAME, minter_addr).unwrap();
    orc.contract_map.add_address(SG721_NAME, sg721_addr).unwrap();
...
pub fn instantiate_infinity_index(
    orc: &mut CosmOrc<CosmosgRPC>,
    user: &SigningAccount,
    marketplace: &str,
) -> Result<InstantiateResponse, ProcessError> {
    orc.instantiate(
        INFINITY_INDEX_NAME,
        &format!("{}_inst", INFINITY_INDEX_NAME),
        &InfinityIndexInstantiateMsg {
...
pub fn instantiate_infinity_router(
    orc: &mut CosmOrc<CosmosgRPC>,
    user: &SigningAccount,
    infinity_index: &str,
) -> Result<InstantiateResponse, ProcessError> {
    orc.instantiate(
        INFINITY_ROUTER_NAME,
        &format!("{}_inst", INFINITY_ROUTER_NAME),
        &InfinityRouterInstantiateMsg {
