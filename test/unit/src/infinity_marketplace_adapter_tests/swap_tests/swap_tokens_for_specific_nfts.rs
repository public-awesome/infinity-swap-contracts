...
#[test]
fn swap_token_for_specific_nfts_marketplace_adapter() {
    let SwapTestSetup {
        vending_template:
    ...
    */
    ...
    let nft_orders: Vec<NftOrder> = owner_token_ids
        .iter()
    ...
    let fund_amount = nft_orders
        .iter()
        .fold(Uint128::zero(), |acc, order| acc + order.amount);
    ...
    let exec_msg = InfinityAdapterExecuteMsg::SwapTokensForSpecificNfts {
        collection: collection.to_string(),
        nft_orders,
        swap_params: SwapParams {
            deadline: expires,
